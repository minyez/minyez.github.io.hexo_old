...
- [ ] 增加代码块复制功能.
- [ ] Bugfix: 点击`Software`的category徽章后显示的文章不全. 一些信息:
  - 从最早的Software博文开始一篇篇重新加入, 出现六篇以上时该catogory就只显示最新的前六篇.
  - archives不受影响.
- [ ] 在侧边栏中加入随机的一句(hitogoto).
...
1. 增加基于`hexo-reference`的wiki浮窗式的脚注.
2. MathJax: 比较各种情况下`*`和`_`的渲染结果. 更改CDN.

### 2019-05-10

1. 增加对不蒜子统计的支持.
2. 去掉了Archives不必要的下拉菜单, 直接进入`archives`页面.

### 2019-05-22

1. 增加背景图片, 位置由YAML中`background`变量指定.

### 2019-05-25

1. 利用`hexo-wordcount`进行全站和页面字数统计